...
import { CreateSudoUserDto, forgetPasswordDto, LoginSudoUserDto, ResetPasswordDto, UpdateSudoUserDto } from "../../types";
...
import { rolecapacityidMiddleware, rolecapacityMiddleware, sudouserProtectMiddleWare } from "./sudouser-middleware";
...
export default class SudouserController implements Controller{
  ...
  private initializeRoutes():void{
     ...
     this.router.post("/",sudouserProtectMiddleWare , rolecapacityMiddleware , this.createSudoUser); 
  	 this.router.post("/login",this.login);
  	 this.router.post("/logout",this.logout);
     this.router.post("/forget-password" , this.forgetPassword);
     this.router.post("/reset-password" , this.resetPassword);
     this.router.get("/:id",sudouserProtectMiddleWare, rolecapacityidMiddleware ,this.getSudoUserById);
     this.router.put("/:id" , sudouserProtectMiddleWare , rolecapacityidMiddleware, this.updateSudoUserById );  
     this.router.delete("/:id" , sudouserProtectMiddleWare , rolecapacityidMiddleware , this.deleteSudoUserById); 
  }
  
   private getAllSudoUsers = expressAsyncHandler(async(req:Request , res:Response)=>{
  ...
  res.clearCookie("sudo_user_auth_jwt", {
      httpOnly: true,
      sameSite: "strict",
    })
    .status(200)
